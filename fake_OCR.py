...
############### Pack txt files, this function is not part of the app but run in kernel for me. ############
...
def unpack_txts(mapping_file: str, target_folder: str, overwrite: bool = True) -> None:
    ...
    - MUST NOT create or delete folders. If a path doesn't exist, let it fail.
    ...
    for i, item in enumerate(records, start=1):
        ...
        # Security: must stay under target_root
        if target_root not in dest.parents:
            raise ValueError(f"Unsafe path detected: {dest}")

        # OPTIONAL clarity: ensure the destination folder already exists (do NOT create it)
        if not dest.parent.exists():
            # Let the default behavior happen (would raise FileNotFoundError on write),
            # or raise explicitly with a clearer message:
        ...
        # Write (will raise FileNotFoundError if parent folder missing—allowed per constraints)
