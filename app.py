...
# works the same as python dictionaries. values can be added via keys or attributes.
# "key" in widget shouold write into session state
print("state:")
print(st.session_state)
print(st.session_state.items())

# callbacks:
# normally, when we hit a button script is rerun from top to bottom.
# on_change: run this extracode before rerunning the script again works with st.slider, st.number_input
# on_click works with st.button and st.form_submit
# both are keywords in these widgets that take a function as argument
...
with (st.sidebar):
    ...
    with tabs[0]:
        ...
        #useless unless we have a state
    ...
    with tabs[5]:
        ...
        #st.toggle("Debug logs", key="set_debug")
        #st.text_input("Weaviate URL (informational)", value="http://localhost:8080",
        #             key="set_weaviate")
...
# if rag_collection and rag_query and rag_go and rag_limit and rag_query and rag_timeout:
#     st.session_state["main_state"] = "go_rag"
...
# handel populate screen
    ...
    if rag_go:
        ...
        try:
            ...
            #rag_query = st.session_state["rag_query"]
