...
def add_new_collection_to_feedback(d: dict, collection_name: str, pop_log, chunk_size, overlap, batch_size=1):
    ...
    if base in d:
        if len(d[base]["rag_feedback_data"]) > 0: # do this only if there is feedback data stored, utherwise useless to keep
            counter = 1
    ...
    objs = [
        DataObject(
            properties={
                ...
                "filepath": pdf_rel_path,   # <-- RELATIVE PATH SAVED HERE
                "language": language,
                "country": country,
                "chunk_index": i,
                "file_id": file_id,
...
def populate_fun(
    ...
    #prob_log_path = os.path.join(os.getcwd(), "feedback_collector", f"populate_{db_name}_logs.txt")
    #st.session_state["pop_data"] = prob_log_path, total, already_seen_list, new_files, duplicates_list, missing_pdf_list
...
def initialize(client, db_name):
    ollama_url = os.getenv("OLLAMA_URL", "http://localhost:11434") # find this via os.
...
def semantic_search(query, client, limit, collection_name):
    # get collection
    collection = client.collections.get(collection_name)  # changed

    # get responses
    response = collection.query.near_text(
        query=query,
        limit=None,                                   # added (you forgot in near_text)
        return_metadata=MetadataQuery(distance=True)   # here distance, in hybrid score
    )
...
if __name__ == "__main__":
    ...
    def add_txt(txt_file_path, collection, pdf_filepath, file_id, chunk_size, overlap, batch_size=1):
        """
        Add text chunks from a txt file to the Weaviate collection, batching inserts
        to avoid Ollama timeout issues.
        ...
        :param txt_file_path: Path to the .txt file
        :param collection: Weaviate collection object
        :param pdf_filepath: Path to the corresponding PDF
        :param file_id: Unique hash of the PDF file
        :param chunk_size: Maximum words per chunk
        :param overlap: Word overlap between chunks
        :param batch_size: Number of chunks per insert_many call
        ...
        pdf_filepath_str = pdf_filepath.as_posix()
        country = "Venezuela"
        language = "Spanish"
...
###############################################################
    ...
    print("this is the updated script 3 - with batchsize = 1 and new add_txt function")
    client = _connect_client()
    try:
        coll = client.collections.get("Test_collection_1")
    	# Try one small txt file
        path = "LEX-FAOC001238.txt"

        add_txt(
            txt_file_path= path,
            collection=coll,
            pdf_filepath=Path("pdf_data/example.pdf"),
            file_id="TEST123",
            chunk_size=50,
            overlap=5,
            batch_size=1
